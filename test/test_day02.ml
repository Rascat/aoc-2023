...
let test_parse_game_line (input : string) (expected : game) =
  let result = parse_game_line input in
  ...
  run
    ...
    [ ( "parse_game_line"
      , [ test_case "parse_game_line" `Quick (fun () ->
            ...
            in
            test_parse_game_line input expected)
        ] )
    ; ( "parse_round"
    ...
    ; "solve_part_one", [ test_case "solve_part_one" `Quick test_solve_part_one ]
    ]
;;
